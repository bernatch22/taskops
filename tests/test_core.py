...
def test_a_project_fact_is_newest_wins_however_the_log_arrives() -> None:
    ...
    property 3 of `replay` has to hold through `project_at` instead. The old
    event is applied LAST here: that is another clone's log being merged in, and
    it must not resurrect the origin somebody already changed."""
    ...
    state = replay.fold([new, old])
    assert state["project"]["remote"] == {"slug": "c/d"}
    assert replay.fold([old, new])["project"] == state["project"]
