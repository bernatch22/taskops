...
Nada requiere intervención humana: no quedó nada bloqueado, nada reclamado en vuelo, y las 8 cards del día cerraron.

El día fue de reordenar la superficie del CLI y sumarle el reporte diario, más dos fixes de comportamiento que salieron de bugs reportados.

**Nombres y agrupación del CLI.** `taskops studio` pasó a llamarse `taskops ui` (`studio/` → `ui/`, `commands/studio.py` → `ui.py`), y de paso los rechazos de read-only y de token ahora nombran el comando que imprime la URL — el rename no se quedó en el subparser. Apareció un dominio `taskops tasks` para gente (list/show/add/plan/done/release/log/search), cada subcomando envolviendo un run existente, mientras el grupo de hooks se fue de `--help` como plumbing. El camino de spawn se renombró a `taskops run`, visible pero con advertencia de facturación, confirmación, `--yes` para llamadores desatendidos y un `--dry-run` gratis: la decisión fue exponerlo bajo un nombre que dice lo que cuesta en vez de esconderlo.

**Reporte del día.** `report day` es lo más grande que entró (+793 −48): `contracts/day.py`, `engine/day.py` con ventana de medianoche local vía `mktime` — así un día con cambio de horario tiene 23 o 25 horas y no se corre. Encima, `report day --write` persiste `.taskops/reports/YYYY-MM-DD.md` con un fingerprint de `max_seq` y una sección `## Narración` vacía, y se niega a sobrescribir salvo `--force`; el skill `/taskops:digest` es el que la llena.

**Edición y listados.** `tasks edit` permite corregir título, spec y prioridad: nuevo tipo de evento `edited` (un evento por campo cambiado) y `TaskTable.set_field` acotado por una whitelist `EDITABLE_FIELDS`, con el SQL quedándose en `storage/`.

**Los dos bugs.** Un proyecto terminado ya no responde con silencio: `tasks` cae a las cards cerradas cuando no hay nada abierto (más nuevas primero, tope de 10 con "+N more"), y suma `--all` y `--status`. Y los workers dejaron de heredar las credenciales de Anthropic — `DROPPED_ENV` (`ANTHROPIC_API_KEY`/`AUTH_TOKEN`/`BASE_URL`) más un parámetro `drop` en `_process.spawn`.
