...
serve-mode host, but only for a board whose CODE is here.** The handler asks
`repos.backed()` for the FACT and serves `PACKAGED` behind the same credential
/rpc asks for; a board with no git on this host keeps the one sentence (`NO_UI`
below, answered `410 Gone`), which names both ways out.
...
board's host is in until code is pushed to it — could only serve a page
...
binary serves the window; the server serves the truth. What ends that state,
since §16's "The host becomes the remote" reversed the amendment that first
ended it, is the board's OWN repository (`<root>/<board>/repo.git`) — created by
the first push, never pruned — and only a board that holds one gets `/ui/` back.
The gate MOVED rather than changed: it used to be the declared forge, whose
mirror was the host's only git; the forge is now a projection of `repo.git` and
opens nothing. `NO_UI` therefore means "nobody has pushed this board's code
here", never "this host can never have a repo" — and, unlike the forge fact, it
is a state every worktree joined to the board clears on its next commit.
...
NO_UI = (
    "this board's window is not served here — no code has been pushed to this "
    "board's git on this host, so there is no history for a dashboard to show "
    "diffs from. Two ways in: run `taskops ui` in a checkout joined to this "
    "board (the window reads your own clone), or push this board's code to "
    "this host and it serves the page itself — a worktree joined to the board "
    "pushes every commit on its own.\n"
)

PACKAGED = Path(__file__).resolve().parent.parent / "ui"
"""The bundle the wheel ships (`src/taskops/ui/`). A WINDOW mounts it via
`Mounts.ui`; a serve-mode host serves the SAME files for a board whose own
repository is here — one copy, never a second bundle path to drift."""
...
def answer(root: Path | None, rest: str) -> tuple[int, bytes, str]:
    ...
    NO window opens for this board here — a board host holding none of that
    board's git — and the module docstring argues why that is a 410 and one sentence
