...
def register(sub: "argparse._SubParsersAction[argparse.ArgumentParser]") -> None:
    ...
    parser.add_argument("what", nargs="?", default="",
                        help="the TITLE for `new` — three or five words — else a milestone id")
    ...
    parser.add_argument("--goal", default="",
                        help="what done means, and what is out of scope. Prose, as long as it needs")
    parser.add_argument("--title", default="", help="with edit: rename it")
...
def run(args: argparse.Namespace) -> str:
    ...
    if args.verb == "new":
        moved = ms.open_chapter(where, what, goal=str(args.goal), horizon=str(args.horizon),
                                planned=bool(args.planned), actor=who)
    elif args.verb == "edit":
        moved = ms.edit(where, what, title=str(args.title), goal=str(args.goal),
                        horizon=str(args.horizon), actor=who)
    elif args.verb == "show":
...
def _move(args: argparse.Namespace, where: Path, who: str, what: str) -> Milestone:
    ...
    if args.verb == "start":
        return ms.start(where, what, actor=who)
    if args.verb == "review":
        return ms.hand_over(where, what, note=note, actor=who)
    if args.verb == "done":
        carry = tuple(part.strip() for part in str(args.carry).split(",") if part.strip())
        return ms.verify(where, what, carry=carry, into=str(args.into), actor=who)
    if args.verb == "reject":
        return ms.send_back(where, what, note=note, actor=who)
    return ms.abandon(where, what, note=note, actor=who)


def _one(where: Path, wanted: str) -> str:
...
def _listing(args: argparse.Namespace, where: Path) -> str:
