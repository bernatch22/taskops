...
`git switch` appears nowhere in this package. A card's branch is pinned to a
...
def merge_card(repo: Path, milestone_branch: str, card_branch: str, card: str) -> str:
    ...
    """
    tree = ensure_milestone(repo, milestone_branch)
    ...
    if not result.ok:
        ...
        raise Refused(
            ...
            # by a re-dispatch (`_worktree` is idempotent), so it cannot be
