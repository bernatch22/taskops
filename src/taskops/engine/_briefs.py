...
def prompt_for(task: Task, actor: str = "") -> str:
    ...
    return (f"You are taskops worker `{actor}`. Do NOT use any other actor id — it is already "
            ...
            f"Do the work, commit it on the branch the claim names, then FINISH with "
            f"`taskops_update task={task['id']} status=review` and a comment naming which "
            f"acceptance criteria you believe you met and what proves each. Somebody else "
            f"closes it — a card you moved to review is not yours to mark done. "
            f"(A card with no acceptance criteria and a trivial change may go straight to "
            f"`status=done` instead; the review step is for work somebody should check.) "
...
def brief_for(root: Path, task: Task, actor: str, tree: Path) -> str:
    ...
    return (
        ...
        f"4. HAND IT ON: `taskops_update repo_path={root} task={task['id']} actor={actor} "
        f"status=review comment='<which acceptance criterion you met, and what proves each — "
        f"a test name, a command, a run>'`. You do NOT close your own card: a `done` on a card "
        f"you yourself put in review is refused, because a review you performed on yourself is "
        f"not one. A verifier — or a human — closes it. (If the card has no acceptance criteria "
        f"and the change is trivial, `status=done` with a comment is still fine.)\n\n"
