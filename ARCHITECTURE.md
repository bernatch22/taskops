...
the actual source (`src/taskops`, 67 files / ~6,500 lines) and tests
(201 passing). This file describes what is *built* — if it ever disagrees with
...
erDiagram
    ...
    EVENT {
        ...
        string actor
        string kind "created|edited|claimed|released|status|comment|commit|merged|milestone"
...
stateDiagram-v2
    ...
    state open {
        ...
        review --> reviewing : taskops_take review=true (the REVIEW lease)
...
flowchart TB
    subgraph L0["0 · foundation (stdlib only)"]
        errors["_errors, _ids, _clock, _json"]
    end
    ...
    subgraph L3["3 · verbs (+ the REGISTRY)"]
        ...
        assign["assign"]; pulse["pulse (board)"]; record["record"]; report["report"]
    ...
    subgraph L5["5 · transports"]
        mcpsrv["mcp/server, tools, dossier, before, render, brief, schema, thread"]
...
only under `store/`, the wall clock only in `_clock.py`, and every module stays
...
`taskops_take review=true` routes to the `review` verb rather than `take`
(one tool for "claim this card and show me everything", two mutexes
underneath); and `taskops_update` + `taskops_comment` are two tools over the ONE `update`
...
§5), deploy via `shipway`, and the repo itself has `git init` but zero
commits.
...
* **Docs are part of the diff.** This file, `README.md`, `CLAUDE.md` and
  `MENTIONS.md` are the whole of the documentation — counts, "not yet" and
  status tables all expire.
