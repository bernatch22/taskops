...
the actual source — `src/taskops`, 80 Python files / ~8,553 lines, plus the
dashboard's own TypeScript source under `ui/src`, which §15 describes and
`CLAUDE.md` gives the command to count (45 files / ~11,646 lines) — and the
tests (`./scripts/test`: 321 passed). Every number here is meant to be re-derived, never
