...
215 passed). Every number here is meant to be re-derived, never
...
harness (`ui/smoke/run.mjs`); `npm run check` runs all four of its steps, and
its `git diff --exit-code` clause reports a bundle that has drifted from `ui/`
until the chapter-close rebuild lands.
