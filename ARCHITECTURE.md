...
the actual source — `src/taskops`, 68 Python files / ~6,700 lines, plus the
...
202 passed, 1 skipped). Every number here is meant to be re-derived, never
...
flowchart TB
    ...
    subgraph L3["3 · verbs (+ the REGISTRY)"]
        ...
        assign["assign"]; pulse["pulse (board, mentions)"]; record["record"]; report["report"]
...
|---|---|---|---|
| `board` | read | both | — |
| `mentions` | read | both | — (the delivery hook's read: the ONE read that renews no lease) |
| `card` | read | both | — |
| `report` | read | both | — |
...
## 6. The nine MCP tools (not the same as the eleven verbs)
...
```
202 passed, 1 skipped   ./scripts/test
...
* **Monitor** — Nova's first and central section and the default tab. Its
  SHELL is built: the two-column layout (`ui/src/pages/Monitor.tsx`) and the
  shared pane chrome (`components/monitor/Pane.tsx`), with every panel's props
  declared as an interface in `components/monitor/panels.ts` — the seam landed
  serialized FIRST, which is `docs/fan-out.md`'s own prescription applied to
  the milestone that produced it. Nova draws **eight** `<section>` panes in six
  layout slots (three stacked blocks left, three panes right); they are stubs
  with honest empty states until one card per panel fills them.
* **Board** — the nine groups of `docs/design.md` §4, as the board reports them.
* **the card dossier drawer**, opening over either tab through `App`'s
...
OS scheme. `tests/test_ui.py` is skipped until the smoke harness
(`ui/smoke/run.mjs`) lands — until then `npm run check` fails at its `smoke`
step and `npm run build` is the step that matters.
