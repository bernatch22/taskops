...
the actual source — `src/taskops`, 114 Python files / ~12,400 lines, plus the
...
tests (`uv run pytest`). Every number here is meant to be re-derived, never
...
## 18. GitHub is the INTRODUCTION, never the credential (2026-08-10)
...
### The gap this close found: nothing DECLARES a forge

`op=forge` is reachable from `verbs/project.py` and from `/rpc`, and that is
all. There is no `taskops board forge` (`taskops board --help` offers
`create`, `ls`, `push`, `visibility` and nothing else), no `board.forge` in
`http/admin.py::REGISTRY`, and no MCP tool — the nine are unchanged. So today
the only way an owner opts a board in is a raw `project` call, which is how
every test in this chapter does it. `http/github.py::NO_FORGE` advertised
`taskops board forge <owner>/<name> --need push` as the way out; that command
does not exist, and the string now names what does. **The chapter is complete
except for the surface that turns it on**, and the human's board carries that
as the next card rather than this section carrying a promise.
